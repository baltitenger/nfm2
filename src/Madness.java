...
public class Madness {
	...
	int im = 0;
	int mxz = 0;
	int cxz = 0;   ///Camera on the xz plane
	float[][] acelf = new float[][]{{4.0f, 2.0f, 1.0f}, {7.0f, 5.0f, 3.0f}, {5.0f, 4.0f, 2.5f}, {5.5f, 4.5f, 3.0f}, {8.0f, 5.0f, 3.5f}, {7.3f, 4.5f, 3.0f}, {7.3f, 4.5f, 3.0f}, {10.0f, 6.0f, 4.0f}, {10.9f, 6.5f, 4.3f}, {10.5f, 7.5f, 5.0f}, {10.2f, 7.1f, 4.7f}, {9.2f, 8.3f, 4.7f}, {12.6f, 8.3f, 3.4f}, {9.5f, 8.5f, 6.0f}, {9.8f, 8.5f, 7.0f}, {8.0f, 7.5f, 6.5f}, {9.5f, 8.3f, 7.5f}, {7.0f, 6.5f, 5.0f}, {9.5f, 9.0f, 8.7f}, {9.6f, 9.1f, 8.8f}, {10.0f, 9.7f, 9.0f}, {11.0f, 10.5f, 9.8f}, {8.0f, 7.8f, 6.8f}, {11.0f, 10.5f, 9.9f}, {11.0f, 10.9f, 10.5f}, {4.0f, 3.5f, 2.8f}, {3.8f, 3.6f, 3.3f}};
	int[][] swits = new int[][]{{50, 130, 210}, {60, 150, 230}, {55, 150, 225}, {60, 155, 240}, {60, 160, 250}, {60, 160, 260}, {60, 160, 265}, {70, 180, 280}, {90, 200, 287}, {90, 200, 292}, {90, 200, 305}, {70, 160, 270}, {110, 240, 282}, {70, 210, 310}, {80, 220, 330}, {50, 220, 310}, {69, 200, 323}, {55, 150, 240}, {80, 220, 338}, {80, 225, 355}, {90, 225, 343}, {90, 225, 362}, {50, 150, 308}, {90, 225, 370}, {90, 240, 395}, {40, 160, 240}, {40, 160, 200}};
	int[] handb = new int[]{9, 10, 9, 10, 10, 10, 9, 10, 11, 11, 10, 8, 8, 10, 6, 11, 9, 12, 9, 10, 13, 11, 11, 14, 14};
	float[] airs = new float[]{2.2f, 2.0f, 1.55f, 1.3f, 1.8f, 1.25f, 1.25f, 0.85f, 1.1f, 1.3f, 0.95f, 0.75f, 0.9f, 1.1f, 1.2f, 0.5f, 0.82f, 0.45f, 0.78f, 0.7f, 1.15f, 0.9f, 0.55f, 1.2f, 1.2f, 0.35f, 0,25f};
	int[] airc = new int[]{50, 0, 0, 0, 0, 0, 0, 0, 20, 20, 10, 10, 10, 10, 20, 0, 5, 0, 5, 5, 20, 15, 0, 20, 25, 0, 0};
	float[] drag = new float[]{0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f};
	int[] turn = new int[]{7, 9, 7, 6, 8, 6, 6, 5, 6, 7, 5, 3, 5, 6, 7, 3, 5, 5, 4, 6, 7, 7, 4, 7, 7, 2, 3};
	float[] grip = new float[]{20.0f, 23.0f, 18.0f, 19.0f, 22.0f, 19.0f, 20.0f, 19.0f, 22.0f, 22.0f, 20.0f, 18.0f, 16.0f, 22.0f, 13.0f, 21.0f, 17.0f, 21.0f, 16.0f, 21.0f, 23.0f, 27.0f, 24.0f, 25.0f, 25.0f, 24.0f, 24.0f};
	...
	public void drive(Control control, ContO conto, Trackers trackers, CheckPoints checkpoints) {
		...
		do {
			if (af2[l4] > 245.0f) {
				...
				int i8 = 0;
				do {
					if (l4 == i8 || !(af2[i8] <= 245.0f)) continue;
					int n = i8;
					af2[n] = af2[n] - (af2[l4] - 250.0f);
				} while (++i8 < 4);
				float f8 = Math.abs(this.m.sin(this.pxy)) + Math.abs(this.m.sin(this.pzy));
		...
		for (int j5 = 0; j5 < trackers.nt; ++j5) {
			...
			do {
				...
				if (trackers.xy[j5] == 0 && trackers.zy[j5] == 0 && trackers.y[j5] != 250 && af2[i7] > (float)(trackers.y[j5] - 5)) {
					...
					int i13 = 0;
					do {
						if (i7 == i13 || !(af2[i13] <= (float)(trackers.y[j5] - 5))) continue;
						int n = i13;
						af2[n] = af2[n] - (af2[i7] - (float)trackers.y[j5]);
					} while (++i13 < 4);
					float f13 = Math.abs(this.m.sin(this.pxy)) + Math.abs(this.m.sin(this.pzy));
				...
				if (trackers.zy[j5] == -90 && af1[i7] < (float)(trackers.z[j5] + trackers.radz[j5]) && this.scz[i7] < 0.0f) {
					...
					int i14 = 0;
					do {
						if (i7 == i14 || !(af1[i14] >= (float)(trackers.z[j5] + trackers.radz[j5]))) continue;
						int n = i14;
						af1[n] = af1[n] - (af1[i7] - (float)(trackers.z[j5] + trackers.radz[j5]));
					} while (++i14 < 4);
					float f14 = Math.abs(this.m.cos(this.pxy)) + Math.abs(this.m.cos(this.pzy));
				...
				if (trackers.zy[j5] == 90 && af1[i7] > (float)(trackers.z[j5] - trackers.radz[j5]) && this.scz[i7] > 0.0f) {
					...
					int i15 = 0;
					do {
						if (i7 == i15 || !(af1[i15] <= (float)(trackers.z[j5] - trackers.radz[j5]))) continue;
						int n = i15;
						af1[n] = af1[n] - (af1[i7] - (float)(trackers.z[j5] - trackers.radz[j5]));
					} while (++i15 < 4);
					float f15 = Math.abs(this.m.cos(this.pxy)) + Math.abs(this.m.cos(this.pzy));
				...
				if (trackers.xy[j5] == -90 && af[i7] < (float)(trackers.x[j5] + trackers.radx[j5]) && this.scx[i7] < 0.0f) {
					...
					int i16 = 0;
					do {
						if (i7 == i16 || !(af[i16] >= (float)(trackers.x[j5] + trackers.radx[j5]))) continue;
						int n = i16;
				...
				if (trackers.xy[j5] == 90 && af[i7] > (float)(trackers.x[j5] - trackers.radx[j5]) && this.scx[i7] > 0.0f) {
					...
					int i17 = 0;
					do {
						if (i7 == i17 || !(af[i17] <= (float)(trackers.x[j5] - trackers.radx[j5]))) continue;
						int n = i17;
						af[n] = af[n] - (af[i7] - (float)(trackers.x[j5] - trackers.radx[j5]));
					} while (++i17 < 4);
