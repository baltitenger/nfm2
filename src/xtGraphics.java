...
implements Runnable {
	...
	public void loadmusic(int i, int j) {
		...
		if (!this.loadedt[i - 1]) {
			this.stracks[i - 1] = new RadicalMod("music/stage" + i + ".radq");
	...
	public void loadimages() {
		...
		this.dnload += 12;
		try {
			File file = new File("data/images.radq");
	...
	public void credits(Control control) {
		...
		if (this.flipo == 101) {
			...
			this.drawcs(153, "This version of the game was updated and is maintained", 0, 0, 0, 3);
			this.drawcs(173, "By Jaroslav Paska (Phyrexian)", 70, 70, 70, 3);
			this.rd.setFont(new Font("SansSerif", 1, 13));
			this.ftm = this.rd.getFontMetrics();
			this.drawcs(210, "Thanks for Game Testing", 0, 0, 0, 3);
			this.rd.setFont(new Font("SansSerif", 1, 11));
			this.ftm = this.rd.getFontMetrics();
			this.drawcs(230, "Soufy H Abutaleb, Sherif Abouzeid,", 90, 90, 90, 3);
			this.drawcs(245, "Kareem Mansour, Youssef Wahby,", 90, 90, 90, 3);
			this.drawcs(260, "Taymour Farid, Mahmoud Waly", 90, 90, 90, 3);
			this.drawcs(275, "and Mahmou7d Ezzeldin (Turbo)", 90, 90, 90, 3);
			this.rd.setFont(new Font("SansSerif", 1, 13));
			this.ftm = this.rd.getFontMetrics();
			this.drawcs(340, "Music was obtained from the ModArchive.org", 0, 0, 0, 3);
			this.rd.setFont(new Font("SansSerif", 1, 11));
			this.ftm = this.rd.getFontMetrics();
			this.drawcs(380, "For any comments: youtube.com/user/PhyrexianNFM", 90, 90, 90, 3);
		}
		if (this.flipo == 102) {
			int j = 0;
			do {
			...
			if (checkpoints.stage == this.unlocked) {
				...
				if (checkpoints.stage != nTracks) {
					...
					if (this.radpx == 147) {
						...
						this.ftm = this.rd.getFontMetrics();
						if (this.aflk) {
							this.drawcs(259, "A Game by Radicalplay.com", 144, 167, 255, 3);
						} else {
							this.drawcs(259, "A Game by Radicalplay.com", 228, 240, 255, 3);
						}
					}
					if (this.aflk) {
			...
			block56: {
				...
				for (int k2 = 1; k2 < byte0; ++k2) {
					...
					while (!aflag[k2]) {
						...
						int i1 = 0;
						do {
							if (k2 == i1 || this.sc[k2] != this.sc[i1]) continue;
							aflag[k2] = false;
						} while (++i1 < 7);
						float f = this.proba[this.sc[k2]];
	...
	public void sparkeng(int i) {
		...
		do {
			if (++i == j) {
				...
				this.engs[this.enginsignature[this.sc[0]]][j].loop(-1);
				this.pengs[j] = true;
				continue;
			}
			if (!this.pengs[j]) continue;
	...
	public void loaddata(int i) {
		...
		this.runner.start();
		this.loadimages();
		this.cars = new RadicalMod("music/cars.radq");
		this.dnload += 27;
		int j = 0;
		do {
			int k = 0;
			do {
				this.engs[k][j] = this.getSound("sounds/" + s + "" + k + "" + j + ".wav");
				this.dnload += 3;
			} while (++k < 5);
			this.pengs[j] = false;
		} while (++j < 5);
		this.stages = new RadicalMod("music/stages.radq");
		this.dnload += 91;
		j = 0;
		do {
	...
	public void clicknow() {
		...
		if (this.aflk) {
			...
			this.drawcs(355, "Click here to Start", 0, 67, 200, 3);
			this.aflk = true;
		}
	}
	...
	public void hipnoload(int i, boolean flag) {
		...
		if (byte0 == 0) {
			...
			if (i == 8) {
				this.rd.drawString("Watch out, EL KING is out to get you now!", 197, 67);
			...
			if (i == 9) {
				this.rd.drawString("It\u2019s good to be the king!", 197, 67);
			}
			if (i == 10) {
